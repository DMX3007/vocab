...
describe('promotion on a correct answer', () => {
  it('moves up one box and grows the interval each time: 2d -> 4d -> 8d -> 16d', () => {
    // A brand-new word starts already "in" box 1 (see initial state above),
    // so its first correct answer is what promotes it to box 2.
  ...
  it('stays in the last box once it is fully promoted (no box 6)', () => {
    let s: SrsState = initialState('leitner', NOW);
  ...
  it('sets dueAt to now + the new box interval', () => {
    ...
    const s1 = leitner.schedule(s0, 4, NOW); // -> box 2, 2 days
    expect(s1.dueAt.getTime()).toBe(NOW.getTime() + days(2));
  });
});
...
describe('demotion on a wrong answer', () => {
  it('drops straight back to box 1 no matter how far it had climbed', () => {
    let s: SrsState = initialState('leitner', NOW);
    for (let i = 0; i < 4; i++) s = leitner.schedule(s, 4, new Date(s.dueAt)); // box 5, 16d
